    ...
    def run(self, text, query, log_file=None) -> str:
        ...
        if self.model_name in ['gemini']:
            ...
            if len(self.enc.encode(self._build_agent_prompt(text, query))) > 12000:
                ...
                if self.travelplannerSolver.memory is not None:
                    for i in range(2):
                        if i == 0:
                            result = self.travelplannerSolver.reasoning(self._build_agent_prompt(text, query),'','',None)
                            # result = self.llm([HumanMessage(content=self._build_agent_prompt(text, query))]).content
                            self.travelplannerSolver.memory(f"The query is: {query}.The generated plan is:\n{result}")
                        else:
                            last_plan = self.travelplannerSolver.memory(query)
                            # result = self.llm([HumanMessage(content=self._build_agent_prompt(text, query)+f"\nThe last plan is:{last_plan}")]).content
                            result = self.travelplannerSolver.reasoning(self._build_agent_prompt(text, query)+f"\nA user demand similar to the current user's travel plan demand and the corresponding travel plan are {last_plan}. Please reflect on the shortcomings of this travel plan, such as where does the plan fail to meet the user's requirements or fail to meet the commen sense, and develop a new travel plan that meets the current user's needs",'','',None)
                            # self.travelplannerSolver.memory(f"The query is: {query}.The generated plan is:\n{result}")
                else :
                    result = self.travelplannerSolver.reasoning(self._build_agent_prompt(text, query),'','',None)
                    # result = self.llm([HumanMessage(content=self._build_agent_prompt(text, query))]).content
