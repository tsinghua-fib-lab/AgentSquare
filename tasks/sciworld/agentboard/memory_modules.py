...
class MEMORY_DILU():
    def __init__(self, llms_type) -> None:
        ...
        db_path = os.path.join(
            './db', 'chroma_5_shot_20_mem0/') if db_path is None else db_path
        # if os.path.exists(db_path):
...
'''),
    ...
    def retrieve_skills(self, query):
        ...
        for doc, _ in docs_and_scores:
            skills.append(str(self.skills[doc.metadata["name"]]["trajectory"]))
        return "\nHere is a similar task and the correct handling trajectory in this case: " + ', '.join(skills)
