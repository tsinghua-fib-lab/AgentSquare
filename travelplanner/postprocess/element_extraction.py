...
if __name__ == '__main__':
    ...
    results = open(f'{args.tmp_dir}/{args.set_type}_{args.model_name}{suffix}_{args.mode}.txt','r', encoding='UTF-8').read().strip().split('\n')
    ...
    idx_number_list = [i for i in range(1,31)]
    for idx in tqdm(idx_number_list[:]):
        generated_plan = json.load(open(f'{args.output_dir}/{args.set_type}/generated_plan_{idx+150}.json'))
        if generated_plan[-1][f'{args.model_name}{suffix}_{args.mode}_results'] not in ["","Max Token Length Exceeded."] and "Day 1" in generated_plan[-1][f'{args.model_name}{suffix}_{args.mode}_results']:
            try:
        ...
        with open(f'{args.output_dir}/{args.set_type}/generated_plan_{idx+150}.json','w') as f:
