...
def transportation_match(text: str):
    ...
    if 'taxi' in str(text).lower():
        return 'Taxi'
    
    elif 'self-driving' in str(text).lower():
        return 'Self-driving'
    
    elif 'flight' in str(text).lower():
        return 'Flight'
...
def is_reasonalbe_visiting_city(question, tested_data):
    ...
    for i in range(min(question['days'],len(tested_data))):
        ...
        if 'from' in city_value:
            ...
            city1 = extract_before_parenthesis(str(city1))
            city2 = extract_before_parenthesis(str(city2))
...
def is_valid_information_in_sandbox(question, tested_data):
    ...
    for i in range(min(question['days'],len(tested_data))):
        ...
        if unit['transportation'] and unit['transportation'] != '-':
            ...
            if 'flight number' in str(value).lower():
                try:
                    org_city = extract_before_parenthesis(str(org_city))
                    dest_city = extract_before_parenthesis(str(dest_city))
                except TypeError:
                    raise ValueError("The transportation {} in day {} can not be parsed.".format(value,i+1))
                # print(value)
                if len(flight.data[(flight.data['Flight Number'] == str(value).split('Flight Number: ')[1].split(',')[0]) & (flight.data['OriginCityName']==org_city) & (flight.data['DestCityName']==dest_city)]) < 1:
                     return False, f"The flight number in day {i+1} is invalid in the sandbox."
            
            elif 'self-driving' in str(value).lower() or 'taxi' in str(value).lower():
                try:
                ...
                if 'self-driving' in str(value).lower():
